...
@router.delete(
    ...
    description=(
        "Soft-delete a student route assignment (is_active = False). "
        "BRANCH_ADMIN or above required."
    ),
)
...
async def mark_attendance(
    ...
    return await assignment_service.mark_attendance(
        db=db,
        trip_id=trip_id,
        school_id=school_id,
        branch_id=branch_id,
...
async def get_trip_attendance(
    ...
    page           : int             = Query(default=1, ge=1),
    page_size      : int             = Query(default=settings.DEFAULT_PAGE_SIZE, ge=1, le=settings.MAX_PAGE_SIZE),
    assignment_type: TripType | None = Query(default=None),
    db: AsyncSession = Depends(get_db),
    current_user: CurrentUser = AnyAuthenticated,
) -> PaginatedAttendanceResponse:
    return await assignment_service.get_trip_attendance(
        db=db,
        trip_id=trip_id,
        school_id=school_id,
        branch_id=branch_id,
...
@router.get(
    ...
    description="Fetch attendance history for a student across trips. Optionally filter by trip_id.",
)
async def get_student_attendance(
    ...
    return await assignment_service.get_student_attendance(
        db=db,
        student_id=student_id,
        school_id=school_id,
        branch_id=branch_id,
...
@router.patch(
    "/trips/{trip_id}/attendance/{attendance_id}",
    response_model=AttendanceResponse,
    status_code=status.HTTP_200_OK,
    summary="Correct attendance record",
    description=(
        "Update the status of an attendance record. "
        "Only the attendance_status can be changed. "
        "BRANCH_ADMIN or above required."
    ),
)
async def update_attendance(
    ...
    return await assignment_service.update_attendance(
        db=db,
